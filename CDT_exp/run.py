...
def func(n0):
    ...
    return reg_theory, reg_auto, reg_op
...
if save:
    ...
    df = pd.DataFrame({'theory' : sum(res1)/rep, 'auto': sum(res2)/rep, 'op' : sum(res3)/rep, 'std_theory': std_theory, 'std_auto': std_auto, 'std_op': std_op})
    df.to_csv(file)
